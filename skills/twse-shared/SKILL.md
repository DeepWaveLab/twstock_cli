---
name: twse-shared
description: "Shared conventions, output formats, and token-saving tips for all twse-cli skills."
version: 1.0.0
metadata:
  category: "foundation"
  requires:
...
# twse-cli Shared Conventions

Common patterns and conventions referenced by all twse-cli skills. Read this before using any other skill.

## Output Format

All `twse` commands return a JSON envelope:
...
4. **Use `--limit`** for sampling before fetching full datasets.
   ```bash
   twse fetch stock.stock-day-all --json --limit 5
   ```
...
Use `twse schema <endpoint> --json` to discover exact field names.

## Error Handling

- **API errors (exit 1):** TWSE may return 503 during high-traffic periods. Wait and retry.
- **Validation errors (exit 2):** Check endpoint name with `twse endpoints --search`.

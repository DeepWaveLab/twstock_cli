...
- If unsure of the code, search: `twse endpoints --search "台積電" --json`
...
```bash
twse fetch stock.stock-day-all --json --code <CODE> --fields "Code,Name,TradeVolume,TradeValue,OpeningPrice,HighestPrice,LowestPrice,ClosingPrice,Change,Transaction" --normalize
```
...
```bash
twse fetch company.t187ap03-l --json --code <CODE> --fields "公司代號,公司名稱,公司簡稱,產業別,董事長,總經理,成立日期,上市日期,實收資本額"
```
...
```bash
twse fetch company.t187ap05-l --json --code <CODE> --fields "資料年月,公司代號,公司名稱,營業收入-當月營收,營業收入-去年當月營收,營業收入-去年同月增減(%)"
```
...
- Verify the code exists: `twse fetch stock.stock-day-all --json --code <CODE> --limit 1`
- Some codes are ETFs or special instruments — try searching: `twse endpoints --search "<CODE>" --json`
...
- [twse-stock-compare](../twse-stock-compare/SKILL.md) — Compare multiple stocks side-by-side
- [twse-company-profile](../twse-company-profile/SKILL.md) — Full company deep-dive with financials
...
- All company endpoint fields use Chinese names. Use `twse schema company.t187ap03-l --json` to discover exact field names.

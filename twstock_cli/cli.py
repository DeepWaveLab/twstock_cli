"""CLI entry point for twstock-cli.

Usage:
    twstock fetch <endpoint> [--json] [--fields F] [--code C] [--limit N]
    twstock endpoints [--json] [--search K] [--category C] [--with-fields]
    twstock stock <command> [options]    # Domain shortcuts
    twstock company <command> [options]  # Domain shortcuts
    twstock broker <command> [options]   # Domain shortcuts
    twstock version
"""
...
class LazyGroup(click.Group):
    ...
    This keeps `twstock --help` fast by not importing 143 command definitions
...
def fetch(endpoint_ref: str | None, as_json: bool, field_list: str | None, stock_code: str | None, max_records: int | None, no_cache: bool, normalize: bool, ndjson: bool, raw: bool, dry_run: bool, use_stdin: bool, date: str | None) -> None:
    ...
    if not ep:
        ...
        console.print("Use [bold]twstock endpoints --search <keyword>[/bold] to discover endpoints.")
...
def endpoints(as_json: bool, keyword: str | None, category: str | None, with_fields: bool) -> None:
    ...
    Examples:
        ...
        twstock endpoints --search "stock.bwibbu" --with-fields --json
    """
...
def schema(endpoint_ref: str, as_json: bool, no_cache: bool, dry_run: bool) -> None:
    ...
    Examples:
        ...
        twstock schema company.t187ap03-l
    """
...
def version() -> None:
    ...
    info = {"name": "twstock-cli", "version": __version__, "python": sys.version.split()[0]}
    if is_agent_mode():
        click.echo(json.dumps(info))
    else:
        console.print(f"twstock-cli {__version__} (Python {info['python']})")
...
def serve() -> None:
    ...
    Exposes twstock-cli as an MCP server for AI agents.
    Tools: twstock_fetch, twstock_endpoints, twstock_schema.
    ...
    try:
        ...
        console.print("Install with: [bold]uv pip install 'twstock-cli[mcp]'[/bold]")
        raise SystemExit(EXIT_VALIDATION_ERROR) from None
